...
class Orden ():
    ...
    def distanciaActual(self,coordenadas):
        ...
        for i in range (len(coordenadas)):
            for line in self.lines:
                ...
                # Convertir las coordenadas en tuplas de enteros
                coord1 = tuple(map(int, coord1.strip('()').split(',')))
                coord2 = tuple(map(int, coord2.strip('()').split(',')))

            if coord1 == self.Aestrella.Obstaculos[coordenadas[i]-1] and coord2 == self.Aestrella.Obstaculos[coordenadas[i]] or coord2 == self.Aestrella.Obstaculos[coordenadas[i-1]] and coord1 == self.Aestrella.Obstaculos[coordenadas[i]]:
                # Si las coordenadas son las deseadas, sumar el valor
                suma += int(value.strip())
                break
        return suma
        # try:
